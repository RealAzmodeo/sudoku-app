...
const initDB = async () => {
  try {
    ...
    // Add author column if missing (Migration equivalent)
    try {
        await pool.query(`ALTER TABLE puzzles ADD COLUMN IF NOT EXISTS author TEXT;`);
    } catch (e) {
...
app.get('/api/puzzles', async (req, res) => {
    const query = `
      ...
      GROUP BY p.id, p.difficulty, p.author, p.createdAt
...
app.get('/api/puzzle/:id', async (req, res) => {
  const { id } = req.params;
  try {
      const { rows } = await pool.query(`SELECT * FROM puzzles WHERE id = $1`, [id]);
      if (rows.length === 0) return res.status(404).json({ error: 'Puzzle not found' });
      
      const row = rows[0];
      res.json({
        id: row.id,
        initialGrid: JSON.parse(row.initialgrid), // Postgres returns lowercase column names by default
...
app.post('/api/score', async (req, res) => {
  ...
  const query = `INSERT INTO scores (puzzleId, playerName, timeSeconds, mistakes) VALUES ($1, $2, $3, $4) RETURNING id`;
  try {
      const { rows } = await pool.query(query, [puzzleId, playerName, timeSeconds, mistakes]);
      res.json({ message: 'Score saved', id: rows[0].id });
  } catch (err) {
      res.status(500).json({ error: err.message });
  }
});
