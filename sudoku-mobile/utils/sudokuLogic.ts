...
export const checkCompletion = (grid: SudokuGrid, row: number, col: number, solvedGrid: number[][]): string[] => {
  const completions: string[] = [];
  
  // Helper to check if a set of cells matches solution
  const isSetComplete = (cells: {r: number, c: number}[]) => {
    return cells.every(({r, c}) => grid[r][c].value === solvedGrid[r][c]);
  };

  // Check Row
  const rowCells = Array(9).fill(0).map((_, i) => ({ r: row, c: i }));
  if (isSetComplete(rowCells)) completions.push('row');

  // Check Col
  const colCells = Array(9).fill(0).map((_, i) => ({ r: i, c: col }));
  if (isSetComplete(colCells)) completions.push('col');

  // Check Box
  const boxStartR = Math.floor(row / 3) * 3;
  const boxStartC = Math.floor(col / 3) * 3;
  const boxCells = [];
  for(let i=0; i<3; i++) {
    for(let j=0; j<3; j++) {
      boxCells.push({ r: boxStartR + i, c: boxStartC + j });
    }
  }
  if (isSetComplete(boxCells)) completions.push('box');
